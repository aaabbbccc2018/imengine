...
int AuthClientHandler::OnDataReceived(std::shared_ptr<PackageMessage> message_data) {
  ...
  evb->runInEventBaseThread([conn_id, message_data]() {
    ...
    if (pipeline) {
      auto handler = pipeline->getHandler<ZProtoHandler>()->CastByGetAttachData<GateServerHandler>();
      handler->OnAuthServerAuthRsp(std::static_pointer_cast<ProtoRpcResponse>(message_data));
    } else {
