...
public:
  ...
  const folly::IOBuf* body() const {
    return packed_data_.get();
  }
  
  void set_body(std::unique_ptr<folly::IOBuf> v) { packed_data_ = std::move(v); }

private:
  std::unique_ptr<folly::IOBuf> packed_data_;
